...
\titleformat{\chapter}[display]% NEW
    {\normalfont\begin{center}}{\chaptertitlename\ \thechapter}{12pt}{}% NEW
\titlespacing*{\chapter}{0pt}{30pt}{20pt}% NEW

%\titleformat{\chapter}[block]{first}{label}{12pt}

\titleformat{\chapter}{}{\thesection}{1em}{}
\titleformat{\section}{}{\thesubsection}{1em}{}
\titleformat{\subsection}{}{\thesubsubsection}{1em}{}
\titleformat{\subsubsection}{}{\theparagraph}{1em}{}
...
\title{A Survey of Web Testing Frameworks and Workflow for Testing JavaScript Applications} %\vfill gives us the black space at the top of the page
...
\section{Problems Still Faced/Why It's So Hard}
...
The developer is not free from all worry as they must still consider differing web browsers. This problem, however, is almost fully remedied by a number of online APIs that will rework underlying code based on browser versions and types. 
...
\subsection{What is the Client-side Portion?}
...
\subsection{What is the Server-side Portion?}
...
One example of a client-server architecture that is popular for the web is Representational State Transfer, also known as REST, architecture. If a client-server architecture meets all of the REST criteria, it is said to be RESTful. \todo{Should I really explain all the constraints?} As this paper is not about Client Server interactions, we will not outline the constraints but instead outline how RESTful APIs look in web applications. The RESTful web APIs use: 1) a base URI, 2) an internet media type, and 3) the standard HTTP methods.
...
\begin{figure}
	...
	\captionfonts
\caption[figure]{Illustration relating phase bug found in with effort required to fix.}
\end{center}
\end{figure}
...
In the following chapter we will discuss the testing frameworks that we looked at and the application we used to evaluate them.
...
\begin{figure}
  ...
  \captionfonts
\caption[todo]{ToDo app home page, simple three-button naviagation}
  \end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{ToDo app task page}
  \end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{ToDo app completed tasks page}
\end{center}
\end{figure}
...
\subsection{Jasmine \cite{Jasmine}}
...
\subsubsection{Assertion Library of Choice: Chai.js \cite{Chaijs}}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Each individual spec is further broken down into the assertions within it.}
\end{center}
\end{figure}
...
\subsection{Intern\cite{InternIO}}
...
\subsection{Selenium\cite{Selenium}}
...
\subsection{Nightwatchjs\cite{NightwatchJS}}
...
\subsection{Jenkins \cite{Jenkins}}
...
\subsection{Semaphore \cite{Semaphore}}
...
\subsection{Drone.io \cite{DroneIO}}
...
\subsection{Sauce Labs\cite{SauceLabs}}
...
\subsection{Istanbul\cite{Istanbul}}
...
\subsection{Grunt.js\cite{GruntJS}}
...
\subsection{Node Inspector\cite{NodeInspector}}
...
\section{PolyXpress (PX)\cite{PX}}
Our major evaluation application was an in-house application called PolyXpress (PX). As the About page says, ``PolyXpress allows you to create location-based stories, build eTours, or create restaurant guides. It is the tool that will bring people to locations in order to entertain, educate, or provide amazing deals.''\cite{PX} Users can play or create ``Stories'', each of which consists of ``Chapters''. Each chapter consists of various ``Events''. Each ``Event'' generally give the user some sort of information, in the form of text, video, audio, or a picture. After the player has chosen a story, they must physically reach certain checkpoints to unlock chapters and events. Because the application requires physical movement, it works solely on devices with GPS, generally phones and tablets. Rather than put a series of pictures showing PolyXpress here, we will put them in the appendix at the end of the paper. To get a better sense of what PolyXpress does and looks like, we recommend looking at the appendix now.
...
\subsection{``A Framework for Automated Testing of JavaScript Web Applications'' \cite{FrameworkForAutomatedTesting}}
...
\subsection{``A Multi-Agent Software Environment for Testing Web-based Applications'' \cite{MultiAgentSoftwareEnvironment}}
...
\subsection{``A 2-Layer Model for the White-Box Testing of Web Applications'' \cite{2LayerModel}}
...
\subsection{``Invariant-Based Automatic Testing of AJAX User Interfaces'' \cite{InvariantBasedUseInterfaces}}
...
\subsection{``JSART: JavaScript Assertion-based Regression Testing'' \cite{JSART}}
...
\subsection{``DOM Transactions for Testing JavaScript'' \cite{DOMTransactions}}
...
\subsection{``Testing Web Applications in Practice'' \cite{TestingInPractice}}
...
\subsection{``Contract-Driven Testing of JavaScript Code'' \cite{ContractDrivenTesting}}
...
\subsection{``JAWS: A JavaScript API for the Efficient Testing and Integration of Semantic Web Services'' \cite{JAWS}}
...
\subsection{``WebMate: A Tool for Testing Web 2.0 Applications'' \cite{WebMate}}
...
\subsection{``Continuous Testing with Ruby, Rails, and JavaScript'' \cite{BookContinuousTesting}}
...
\subsection{``Finding Bugs In Dynamic Web Applications'' \cite{FindingBugs}}
...
\subsection{``Testing Web Services: A Survey'' \cite{TestingWebServicesSurvey}}
...
\subsection{``A Framework for Testing RESTful Web Services'' \cite{RESTfulFramework}}
...
\subsection{``Going Faster: Testing The Web Application'' \cite{GoingFaster}}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Story selection}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Chapter pane with approximate distances to each chapter}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Each page and element of PolyXpress has instructions, here they are for Chapters}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{A map of the chapter location in relation to the player}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{PolyXpress marketplace, to find other published stories}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{A map centered around the player}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{PolyXpress Authoring Tool}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Form to create an Event}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Bottom of form to create an Event}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Form for creating Chapters}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Events are selected manually for inclusion in Chapters}
\end{center}
\end{figure}
...
\begin{figure}
  ...
  \captionfonts
\caption[figure]{Story creation asks if the user wants their story published}
\end{center}
\end{figure}
