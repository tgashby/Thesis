...
Mocha, by default, also uses the \lstinline{describe -> it(\"should...\"} pattern to create tests. Mocha offers a variety of other test interfaces for developer's to write their test code the way they want to in addition to a custom, nearly unstructured variety. A few of the testing interfaces are outlined here:
\begin{lstlisting}
// BDD (default)
...
Mocha's approach to asynchronous testing is quite a bit more straightforward than Jasmine's. In the \lstinline{it(...)} anonymous function, there is passed an additional parameter called \lstinline{done} which is then called when the asynchronous call has finished. This required us to edit the code we were testing to make all the async calls accept an additional callback, but it was a very small price to pay for extended testability. An example asynchronous test would look like this:
\begin{lstlisting}
...
%-AMD
Intern uses Asynchronous Module Definition (AMD) form which is something we'd not seen before. Where this comes into play is merely in what used to be \lstinline{require(...)}. Instead of seperately requiring modules, the entire program is wrapped in a \lstinline{define([...])} block that passes modules as parameters to an overarching function. That will make more sense after an example:
\begin{lstlisting}
// Intern
define([
    'intern!object',
...
\lstinline{mocha --} specifies that the executable to be run is mocha and that the developer is done giving commandline args to istanbul, that's the \lstinline{--}
...
The Author is a tool to create one's own stories. Because our tests revolved around the Author, we'll be discussing it in depth in the following section, so to avoid redundency, we'll say nothing else here.
...
Sauce Labs, the online web testing site, offered a phenonemal set of features that we did not have time to take advantage of. The ability to test web applications across multiple browsers and browser configurations without having to set all of them up is incredibly enticing.
